...
#     new VariableNode token.value
# parseString = (token) ->
#     new StringNode token.value
...
#     results
# splitOnComma = (tokens) ->
#     leveledSplit tokens, (t) -> t.type is "Comma"

# parseFunctionCall = (lastNode) ->
...
shift = (ctx) -> tokens: ctx.tokens[1..], index: ctx.index + 1
fail = ({ reason, context }) -> success: no, reason: reason, context: context
success = ({ value, context }) -> success: yes, value: value, context: context

type = (ty) -> (ctx) ->
    if ctx.tokens[0]?.type is ty then success value: ctx.tokens[0], context: shift ctx else fail reason: "Expected a #{ty}", context: ctx
value = (val) -> (ctx) ->
    if ctx.tokens[0]?.value is val then success value: ctx.tokens[0], context: shift ctx else fail reason: "Expected #{val}", context: ctx
...
seq = (...parsers) -> (ctx) ->
    ...
    for parser from parsers
        ...
        { value: node, context: rest } = result
        nodes.push node
    success value: nodes, context: rest

any = (...parsers) -> (ctx) ->
    for parser from parsers
        result = parser ctx
        return result if result.success
        bestError = result if not bestError or result.context.index > bestError.context.index
    bestError

many = (parser) -> (ctx) ->
    res = ((ctx = context; val) while ({ value: val, context } = parser ctx).success)
    success value: res, context: ctx

many1 = (parser) -> fmap (seq parser, many parser), ([first, rest]) -> [first].concat rest

optional = (parser) -> any parser, (ctx) -> success value: null, context: ctx

fmap = (parser, fn) -> (ctx) ->
    if (result = parser ctx).success then success value: (fn result.value), context: result.context else result

operators = (operators) -> any ...(value operator for operator from operators)

exprnode = ([first, rest]) -> new ExprNode first, rest[0][1], rest[0][0].value
expr = (ctx) -> (any (fmap (seq comparison, many1 seq (value "="), expr), exprnode), comparison) ctx
comparison = (ctx) -> (any (fmap (seq binary, many1 seq (operators ["==", "!=", "<=", ">=", "<", ">"]), comparison), exprnode), binary) ctx
binary = (ctx) -> (any (fmap (seq shifts, many1 seq (operators ["^", "&", "|"]), binary), exprnode), shifts) ctx
shifts = (ctx) -> (any (fmap (seq plus, many1 seq (operators ["<<", ">>"]), shifts), exprnode), plus) ctx
plus = (ctx) -> (any (fmap (seq mult, many1 seq (operators ["+", "-"]), plus), exprnode), mult) ctx
mult = (ctx) -> (any (fmap (seq power, many1 seq (operators ["*", "/", "%"]), mult), exprnode), power) ctx
power = (ctx) -> (any (fmap (seq term, many1 seq (operators ["**"]), power), exprnode), term) ctx
term = (ctx) -> (any call, literal, (seq (value "("), expr, value ")")) ctx

args = (ctx) -> (seq expr, many seq (value ","), expr) ctx
array_literal = (ctx) -> (seq (value "["), (optional args), value "]") ctx
number_literal = (ctx) -> (fmap (type "Number"), (n) -> new NumberNode Number n.value) ctx
variable = (ctx) -> (fmap (type "Identifier"), (n) -> new VariableNode n.value) ctx
string_literal = (ctx) -> (fmap (type "String"), (s) -> new StringNode s.value) ctx
literal = (ctx) -> (any number_literal, variable, string_literal, array_literal) ctx #, map_literal
call = (ctx) -> (fmap (seq literal, many1 seq (value "("), (optional args), value ")"), ([name, calls]) ->
    node = name; (node = new CallNode node, calls[0][1] or []; calls = calls[1..]) while calls.length; node) ctx

console.log (expr tokens: [(new Token "a", "Identifier"), (new Token "(", "ParenLeft"), (new Token "(", "ParenLeft"), (new Token ")", "ParenRight")], index: 0).value
...
#     level = 0; tokens = []
#     loop
#         if not input.length then return delimitedBy input
#         if not failed (delimitedBy input) and level is 0
#             [_, input...] = input
#             break
#         if input[0].value in ["If", "While", "For", "Match", "Func", "When", "Otherwise", "Until"] then level++
#         if input[0].value is "End" then level--
